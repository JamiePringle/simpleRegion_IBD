...
def backPaths(EbackCDF,startVec,maxAge,nGen):
    rng = random.default_rng()
    paths=zeros((len(startVec),nGen),dtype=dtype('int16'))
    paths[:,0]=startVec
    for np in range(len(startVec)):
        theRands=random.uniform(size=(nGen+1,1))
        ng=1
        while (ng<nGen):
            #pick a random lifetime, and fill in that number of generations with the location.
            nLifeTime=1 if maxAge==1 else rng.integers(0,maxAge+1)
            paths[np,ng:ng+nLifeTime]=bisect.bisect_left(EbackCDF[:,paths[np,ng-1]],theRands[ng])
            ng=ng+nLifeTime
            #assert paths[np,ng]<EbackCDF.shape[1],'think'
    print('CJMP',paths.shape,'maxAge is',maxAge)
    return(paths)
...
#@profile
def twoTrackCheck(point1,point2,nPatch,nGens,nPaths,maxAge,EbackCDF,nRun):
    ...
    maxAge is the number of generations the species lives
    ...
    paths1=backPaths(EbackCDF,startVec,maxAge,nGens)

    startVec=point2+zeros((nPaths,),dtype=dtype('int16'))
    paths2=backPaths(EbackCDF,startVec,maxAge,nGens)
...
if __name__=='__main__':
    ...
    for loopParam in [1,2,5]:
        #we can iterate over param1 and param2
        #what they do can only be determined by reading the code below
        #nPatch is the population of each habitat patch, default is 5
        #dOff is seperation between points, default is 200

        param1vec=linspace(5.0,100,32); param1name='Ladv'; param2=10.0; param2name='Ldiff'
        nPatch=5; Np=1024; dOff=200; dOffInit=50; maxAge=loopParam
        runName='largeDomain_Ladv_dOff_%d_dOffInit_%d_Ldiff_%d_nPatch_%d_maxAge_%d'%(dOff,dOffInit,param2,nPatch,maxAge);  
        ...
        for param1 in param1vec:
            ...
            startPoints=[(Np//2-dOffInit//2,Np//2+dOffInit//2,'Middle start'),
                         (Np//2-(dOff+dOffInit)//2,Np//2+(dOff+dOffInit)//2,'Seperated start')]
            ...
            for n1 in range(len(startPoints)):
                ...
                nRun+=1
                argVec.append((point1,point2,nPatch,nGens,nPaths,maxAge,EbackCDF,nRun))
            ...
            subplot(2,2,1)
            for n in range(len(startPoints)):
                plot(param1vec,1-fracCoalescVec[:,n],'*-',label=startPoints[n][2])
            xlabel(param1name)
            ...
            # subplot(2,1,1)
            # for n in range(len(startPoints)):
            #     plot(param1vec,1.0-fracCoalescVec[:,n],'*-',label=startPoints[n][2])
            # xlabel(param1name)
            # ylabel('1-fracCoalesc')
            # legend(fontsize='small')
            ...
            #subplot(2,1,2)
            ibdVec=(meanTimeVec[:,1]-meanTimeVec[:,0])
            plot(log10(param1vec),log10(ibdVec),'*-')
            xlabel('log10('+param1name+')')
            ylabel('log 10 change in mean time to MRCA')

            suptitle('IBD for Np=%d and dOff=%d, dOffInit=%d'%(Np,dOff,dOffInit))
            ...
            for n in range(nOutputs):
                ...
                if False: #just do line plots
                    ...
                    plot(param2**2/param1vec,param1vec,'r-*')
                    axis(xmax=Np-1)
            ...
            if True:
                ...
                dictToSave={'answerVec':answerVec,'startPoints':startPoints,'param1vec':param1vec,'param1name':param1name,
                            'nPatch':nPatch,'Np':Np,'dOff':dOff,'param2':param2,
                            'param2name':param2name,'dOffInit':dOffInit,'maxAge':maxAge}
