...
if __name__=='__main__':
    ...
    points2list1=[Np//2-50,Np//2+50] #start far appart together
    ...
    for nFrame in range(nGens):
        ...
        if True:
            ...
            if nFrame in frames2plot:
                ...
                thisPop11=output1[0][:,nFrame].reshape((nPaths,1)) #KernelDensity expects 2D array
                thisSmooth11=makePDFs(thisPop11)
                thisPop12=output1[1][:,nFrame].reshape((nPaths,1)) #KernelDensity expects 2D array
                thisSmooth12=makePDFs(thisPop12)
                thisSmoothOverlap1=thisSmooth11+0.0
                indx=thisSmooth12<thisSmooth11; thisSmoothOverlap1[indx]=thisSmooth12[indx]
                
                thisPop21=output2[0][:,nFrame].reshape((nPaths,1)) #KernelDensity expects 2D array
                thisSmooth21=makePDFs(thisPop21)
                thisPop22=output2[1][:,nFrame].reshape((nPaths,1)) #KernelDensity expects 2D array
                thisSmooth22=makePDFs(thisPop22)
                thisSmoothOverlap2=thisSmooth21+0.0;
                indx=thisSmooth22<thisSmooth21; thisSmoothOverlap2[indx]=thisSmooth22[indx]

                x=arange(Np)
                
                subplot(len(frames2plot),2,2*(nPlots-1)+1)
                plot(thisSmooth11,'r-')
                plot(thisSmooth12,'r-')
                fill_between(x,thisSmoothOverlap1,color='r',alpha=0.5)
                #plot(thisSmooth2,'r-')
                ...
                plot(thisSmooth21,'r-')
                plot(thisSmooth22,'r-')
                fill_between(x,thisSmoothOverlap2,color='r',alpha=0.5)
                plot(points2list1,0*array(points2list1),'r*',markersize=10)
                #ylabel('gen %d'%(nFrame))
                axis(xmin=0.0,xmax=Np-1,ymin=-0.005,ymax=0.065)
                ax=gca()
    ...
    savefig('figTwoLineages.png',dpi=300)
