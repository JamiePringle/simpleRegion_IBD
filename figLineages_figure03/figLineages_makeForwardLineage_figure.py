...
if __name__=='__main__':
    ...
    pop1vec=zeros((Np,)); pop1vec[Np//2]=1.0
    pop2vec=zeros((Np,)); pop2vec[Np//2]=1.0
    
    
    nPlots=0
    #clf()
    for nFrame in range(nGens):
        thisSmooth1=pop1vec #rename so I can copy code while sitting in middle school concert...
        thisSmooth2=pop2vec #
        points2list1=[Np//2]
        ...
        if False:
            ...
            if nFrame in frames2plot:
                ...
                if nFrame==0:
                    title('forwards in time')

                jnk=gca()
                #jnk.get_yaxis().set_visible(False)
                if nFrame!=200:
                    jnk.set_xticklabels(' ')
...
savefig('figLineages.png',dpi=300)
